...
export default function TehsildarOfficePortal() {
  ...
  const loadTehsildarQueue = async () => {
    setLoading(true);
    try {
      const localApps = getOfficeApplicationsFromStore("Tehsildar");
      setQueue(localApps);
      ...
      console.warn("Using offline master store for Tehsildar queue:", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAction = async (target, action) => {
    ...
    const verifiedBy = "Tehsildar Executive Officer";
    const officerRemarks = note || `Tehsildar Executive action: ${action.toUpperCase()}`;
    ...
    await loadTehsildarQueue();

    const statusText = action === "approve" 
      ? (updated?.status === "Approved" ? "Sanctioned & Certificate Issued" : `Stage Cleared → Moved to ${updated?.currentOffice || "Next Office"}`)
  ...
  const servicesList = [
    ...
    "Income Certificate (Annual Revenue)",
    "Caste & Tribe Certificate Verification",
    "Solvency Certificate",
    "Encumbrance & Land Rights NOC",
    "Magistrate Caste Affidavit",
    "Freedom Fighter / Senior Citizen Pension",
    "Natural Calamity Relief Assessment"
  ];
  ...
  return (
    <div style={{ background: "#f8fafc", minHeight: "92vh", padding: "24px" }}>
      <div style={{ maxWidth: "1300px", margin: "0 auto" }}>
        ...
        {/* Header Banner */}
        <div style={{ background: "linear-gradient(135deg, #1e1b4b 0%, #4338ca 100%)", color: "white", padding: "28px", borderRadius: "16px", marginBottom: "24px", boxShadow: "0 10px 25px -5px rgba(30, 27, 75, 0.3)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
            <div>
              <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                <span style={{ background: "#c7d2fe", color: "#1e1b4b", padding: "4px 12px", borderRadius: "12px", fontSize: "0.78rem", fontWeight: "900" }}>
                  OFFICE 2 OF 4 — STANDALONE TEHSILDAR PORTAL
                </span>
              </div>
              <h1 style={{ margin: "8px 0 4px 0", fontSize: "2rem", fontWeight: "900", color: "#ffffff" }}>
                ⚖️ Tehsildar Executive Office Portal
              </h1>
              <p style={{ margin: 0, color: "#e0e7ff", fontSize: "0.95rem" }}>
                Executive Magistrate &amp; Revenue Sub-Division Administration. Approval for Domicile, Income, Caste, Solvency, and Land Rights NOC.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* 🔍 Embedded Master Dataset Search Bar for Tehsildar Officers */}
        <MasterVerificationSearchBar officeName="Tehsildar" themeColor="#4338ca" />

        {/* Master Scope Box */}
        <div style={{ background: "#ffffff", borderRadius: "14px", padding: "20px", border: "1px solid #cbd5e1", marginBottom: "24px" }}>
          <h3 style={{ margin: "0 0 12px 0", fontSize: "1.1rem", fontWeight: "800", color: "#1e1b4b" }}>
            Tehsildar Master Scope &amp; Statutory Services:
          </h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "10px" }}>
            {servicesList.map((s) => (
              <div key={s} style={{ background: "#f5f3ff", border: "1px solid #ddd6fe", color: "#5b21b6", padding: "10px", borderRadius: "8px", fontSize: "0.82rem", fontWeight: "700" }}>
                • {s}
              </div>
            ))}
          </div>
        ...
        <div style={{ background: "#ffffff", borderRadius: "16px", padding: "24px", border: "1px solid #cbd5e1", marginBottom: "24px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "14px" }}>
            ...
            <div style={{ position: "relative", minWidth: "280px" }}>
              ...
              <input
                type="text"
                placeholder="Search Tehsildar Applications..."
                value={searchTerm}
            ...
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  border: "2px solid",
                  borderColor: filterTab === "pending" ? "#6366f1" : "#cbd5e1",
                  background: filterTab === "pending" ? "#e0e7ff" : "#ffffff",
                  color: filterTab === "pending" ? "#4338ca" : "#475569",
                  fontWeight: "800",
                  cursor: "pointer",
                  fontSize: "0.85rem"
                }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(400px, 1fr))", gap: "24px", marginTop: "20px" }}>
              <div style={{ background: "#f0fdf4", borderRadius: "14px", padding: "18px", border: "1px solid #bbf7d0" }}>
                <h4 style={{ margin: "0 0 16px 0", fontSize: "1.1rem", fontWeight: "900", color: "#15803d", display: "flex", alignItems: "center", gap: "8px" }}>
                  <FaCheckCircle /> Approved Tehsildar Services ({approvedList.length})
                </h4>
                {approvedList.length === 0 ? (
                  <div style={{ padding: "20px", color: "#64748b", textAlign: "center" }}>No approved Tehsildar certificates found.</div>
                ) : (
                  <div style={{ display: "grid", gap: "14px" }}>
                    {approvedList.map((app) => (
                      <TehsildarCard key={app._id || app.applicationId} app={app} remarks={remarks} setRemarks={setRemarks} handleAction={handleAction} isApprovedCard />
                    ))}
                  </div>
                )}
              </div>
              ...
              <div style={{ background: "#fef2f2", borderRadius: "14px", padding: "18px", border: "1px solid #fecaca" }}>
                <h4 style={{ margin: "0 0 16px 0", fontSize: "1.1rem", fontWeight: "900", color: "#991b1b", display: "flex", alignItems: "center", gap: "8px" }}>
                  <FaTimesCircle /> Rejected Tehsildar Services ({rejectedList.length})
                </h4>
                {rejectedList.length === 0 ? (
                  <div style={{ padding: "20px", color: "#64748b", textAlign: "center" }}>No rejected Tehsildar certificates found.</div>
                ) : (
                  <div style={{ display: "grid", gap: "14px" }}>
                    {rejectedList.map((app) => (
                      <TehsildarCard key={app._id || app.applicationId} app={app} remarks={remarks} setRemarks={setRemarks} handleAction={handleAction} isRejectedCard />
                    ))}
                  </div>
                )}
              </div>
            ...
            loading ? (
              ...
              <div style={{ display: "grid", gap: "18px" }}>
                {displayedQueue.map((app) => (
                  <TehsildarCard key={app._id || app.applicationId} app={app} remarks={remarks} setRemarks={setRemarks} handleAction={handleAction} />
                ))}
              </div>
            )
          )}
  ...
  return (
    <div style={{ background: cardBg, borderRadius: "14px", border: `2px solid ${cardBorder}`, padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.03)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "10px", marginBottom: "12px" }}>
        <div>
          <span style={{ background: "#e0e7ff", color: "#4338ca", padding: "4px 10px", borderRadius: "6px", fontSize: "0.78rem", fontWeight: "800", marginRight: "8px" }}>
            {app.applicationId}
          </span>
          <span style={{ background: "#f1f5f9", color: "#475569", padding: "4px 10px", borderRadius: "6px", fontSize: "0.78rem", fontWeight: "700" }}>
            {app.serviceType || "Tehsildar Service"}
          </span>
        ...
        <div>
          {isApproved && (
            <span style={{ background: "#dcfce7", color: "#15803d", padding: "6px 14px", borderRadius: "20px", fontWeight: "900", fontSize: "0.85rem", display: "inline-flex", alignItems: "center", gap: "6px" }}>
              <FaCheckCircle /> SANCTIONED &amp; SIGNED
            </span>
          )}
          {isRejected && (
      ...
      {isPaidDues && (
        <div style={{ background: "#ecfdf5", border: "1px solid #a7f3d0", padding: "12px", borderRadius: "10px", marginBottom: "14px", fontSize: "0.85rem", color: "#065f46" }}>
          <div style={{ fontWeight: "800", display: "flex", alignItems: "center", gap: "6px" }}>
            <FaReceipt /> Revenue Dues Cleared: ₹{app.pendingDues.amount} ({app.pendingDues.paymentMethod})
          </div>
      ...
      {isApproved && app.issuedCertificate && (
        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", padding: "12px", borderRadius: "10px", marginBottom: "14px", fontSize: "0.82rem", color: "#166534" }}>
          ...
          <strong>Digital Seal &amp; Signature:</strong> {app.issuedCertificate.certificateId} | {app.issuedCertificate.digitalSignature}
        </div>
      )}
      ...
      {!isApproved && !isRejected && (
        <div style={{ marginTop: "14px", paddingTop: "14px", borderTop: "1px solid #e2e8f0" }}>
          <input
            type="text"
            placeholder="Add Tehsildar Executive Remarks / Reason..."
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            ...
            >
              <FaCheckCircle /> Grant Tehsildar Sanction
            </button>
